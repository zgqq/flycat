...
parser.add_argument("--conf", default=None, help="", required=True)
...
def get_config_value(data, key, env, default_value=None):
    ...
    if "modules" in data.keys() and len(data["modules"].keys()) > 0:
        ...
        if isDebug:
            print(
                "modules: %s, module_name:%s, read config module:%s, key:%s, env:%s"
                % (data["modules"].keys(), module_name, config_name, key, env)
            )
